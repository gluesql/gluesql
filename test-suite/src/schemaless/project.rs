...
test_case!(project, {
    ...
    g.run("CREATE TABLE A");
    g.run("CREATE TABLE B");
    g.run("CREATE TABLE S (a_id INTEGER, tag TEXT);");

    g.run(r#"INSERT INTO A VALUES ('{"a_id":1,"a":"left"}');"#);
    g.run(r#"INSERT INTO B VALUES ('{"b_id":10,"a_id":1,"b":"right"}');"#);
    g.run("INSERT INTO S VALUES (1, 'schema');");
    ...
    {
        g.run("CREATE TABLE C (_doc INTEGER);");
        g.run("INSERT INTO C VALUES (7);");
