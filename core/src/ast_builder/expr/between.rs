...
impl ExprNode {
    pub fn between<T: Into<Self>>(self, low: T, high: T) -> Self {
        Self::Between {
            expr: Box::new(self),
            negated: false,
    ...
    pub fn not_between<T: Into<Self>>(self, low: T, high: T) -> Self {
        Self::Between {
            expr: Box::new(self),
            negated: true,
            low: Box::new(low.into()),
...
mod tests {
    ...
    fn between() {
        ...
        let actual = col("date").between(text("2022-01-01"), text("2023-01-01"));
        let expected = "date BETWEEN '2022-01-01' AND '2023-01-01'";
        test_expr(actual, expected);

        let actual = col("num").not_between(1, 10);
