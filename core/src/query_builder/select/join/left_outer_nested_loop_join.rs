use {
    super::{left_outer_hash_join::LeftOuterHashJoinNode, table_factor, table_source_plan},
    crate::{
        ...
        query_builder::{
            ...
            InnerHashJoinNode, InnerJoinConditionNode, InnerNestedLoopJoinNode,
            LeftOuterJoinConditionNode, LimitNode, OffsetNode, OrderByExprList, ProjectNode,
...
#[derive(Clone, Debug)]
pub struct LeftOuterNestedLoopJoinNode<'a> {
    prev_node: PrevNode<'a>,
    right_name: String,
    right_alias: Option<String>,
}

impl<'a> LeftOuterNestedLoopJoinNode<'a> {
    ...
    pub fn on<T: Into<ExprNode<'a>>>(self, expr: T) -> LeftOuterJoinConditionNode<'a> {
        LeftOuterJoinConditionNode::new(self, expr)
    }
    ...
    pub fn hash_executor<T: Into<ExprNode<'a>>, U: Into<ExprNode<'a>>>(
        ...
        match prev_node {
            PrevNode::Select(node) => {
                LeftOuterHashJoinNode::new(node, right_name, right_alias, right_key, input_key)
            }
    ...
    pub(super) fn build_left_outer_join_plan(self) -> Result<LeftOuterJoinPlan> {
        self.build_nested_loop_join_plan()
            .map(LeftOuterJoinInputPlan::NestedLoop)
            .map(|input| LeftOuterJoinPlan { input })
    }
    ...
    pub fn alias_as(self, table_alias: &'a str) -> SourceNode<'a> {
        QueryNode::LeftOuterNestedLoopJoinNode(self).alias_as(table_alias)
    }
}
...
impl BuildAggregationInputPlan for LeftOuterNestedLoopJoinNode<'_> {
...
impl BuildProjectInputPlan for LeftOuterNestedLoopJoinNode<'_> {
...
impl BuildSelect for LeftOuterNestedLoopJoinNode<'_> {
    fn build_select(self) -> Result<Select> {
...
mod tests {
    ...
    #[test]
    fn successors() {
        let actual = table("A").select().left_join("B").join("C");
        let expected = "SELECT * FROM A LEFT JOIN B JOIN C";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").join_as("C", "c");
        let expected = "SELECT * FROM A LEFT JOIN B JOIN C AS c";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").left_join("C");
        let expected = "SELECT * FROM A LEFT JOIN B LEFT JOIN C";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").left_join_as("C", "c");
        let expected = "SELECT * FROM A LEFT JOIN B LEFT JOIN C AS c";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").project("A.id");
        let expected = "SELECT A.id FROM A LEFT JOIN B";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").group_by("A.id");
        let expected = "SELECT * FROM A LEFT JOIN B GROUP BY A.id";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").having("TRUE");
        let expected = "SELECT * FROM A LEFT JOIN B HAVING TRUE";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").offset(1);
        let expected = "SELECT * FROM A LEFT JOIN B OFFSET 1";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").limit(1);
        let expected = "SELECT * FROM A LEFT JOIN B LIMIT 1";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").filter("A.id > 0");
        let expected = "SELECT * FROM A LEFT JOIN B WHERE A.id > 0";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").order_by("A.id");
        let expected = "SELECT * FROM A LEFT JOIN B ORDER BY A.id";
        test_query_builder(actual, expected);

        let actual = table("A").select().left_join("B").distinct();
        let expected = "SELECT DISTINCT * FROM A LEFT JOIN B";
        test_query_builder(actual, expected);
        ...
        let actual = table("A")
            ...
            .alias_as("Joined")
            .select();
        let expected = "SELECT * FROM (SELECT * FROM A LEFT JOIN B) Joined";
        test_query_builder(actual, expected);
    }
