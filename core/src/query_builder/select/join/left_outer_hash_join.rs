use {
    super::{left_outer_nested_loop_join::LeftOuterNestedLoopJoinNode, table_source_plan},
    crate::{
        ast::Select,
        plan::{
            AggregationInputPlan, HashJoinInputPlan, HashJoinPlan, LeftOuterJoinInputPlan,
            LeftOuterJoinPlan, ProjectInputPlan,
        },
        query_builder::{
...
#[derive(Clone, Debug)]
pub struct LeftOuterHashJoinNode<'a> {
...
impl<'a> LeftOuterHashJoinNode<'a> {
    ...
    pub(super) fn build_left_outer_join_plan(self) -> Result<LeftOuterJoinPlan> {
        self.build_hash_join_plan()
            .map(LeftOuterJoinInputPlan::Hash)
    ...
    pub fn alias_as(self, table_alias: &'a str) -> SourceNode<'a> {
        QueryNode::LeftOuterHashJoinNode(self).alias_as(table_alias)
    }
}

impl BuildFilterInputPlan for LeftOuterHashJoinNode<'_> {
...
impl BuildAggregationInputPlan for LeftOuterHashJoinNode<'_> {
...
impl BuildProjectInputPlan for LeftOuterHashJoinNode<'_> {
...
impl BuildSelect for LeftOuterHashJoinNode<'_> {
...
mod tests {
    use {
        crate::{
            plan::{
                ...
                HavingPlan, LeftOuterJoinInputPlan, LeftOuterJoinPlan, LimitInputPlan, LimitPlan,
            ...
            query_builder::{
                Build, QueryBuilderError, QueryNode, col, expr, num, select::BuildQuery, table,
            },
            result::Error,
        },
    ...
    #[test]
    fn join_successors() {
        let expected_input = LeftOuterJoinPlan {
            input: LeftOuterJoinInputPlan::Hash(HashJoinPlan {
                input: HashJoinInputPlan::Source(SourcePlan::Table(TableSourcePlan {
                    name: "A".to_owned(),
                    alias: None,
        ...
        let actual = table("A")
            ...
            .build_inner_join_plan()
            .unwrap();
        let expected = crate::plan::InnerJoinPlan {
            input: crate::plan::InnerJoinInputPlan::NestedLoop(NestedLoopJoinPlan {
                input: NestedLoopJoinInputPlan::LeftOuterJoin(Box::new(expected_input.clone())),
                right: SourcePlan::Table(TableSourcePlan {
                    name: "C".to_owned(),
                    alias: None,
                    access: TableAccessPlan::FullScan,
        ...
        let expected = LeftOuterJoinPlan {
            input: LeftOuterJoinInputPlan::NestedLoop(NestedLoopJoinPlan {
                input: NestedLoopJoinInputPlan::LeftOuterJoin(Box::new(expected_input)),
                right: SourcePlan::Table(TableSourcePlan {
                    name: "C".to_owned(),
                    alias: Some(TableAliasPlan {
                        name: "c".to_owned(),
    ...
    fn terminal_successors() {
        ...
        let expected = Ok(StatementPlan::Query(QueryPlan::Project(ProjectPlan {
            input: ProjectInputPlan::LeftOuterJoin(Box::new(join.clone())),
        ...
        let expected = Ok(StatementPlan::Query(QueryPlan::Project(ProjectPlan {
            input: ProjectInputPlan::Aggregation(AggregationPlan {
                input: AggregationInputPlan::LeftOuterJoin(Box::new(join.clone())),
        ...
        let expected = Ok(StatementPlan::Query(QueryPlan::Project(ProjectPlan {
            input: ProjectInputPlan::Filter(FilterPlan {
                input: FilterInputPlan::LeftOuterJoin(Box::new(join.clone())),
        ...
        let expected = Ok(StatementPlan::Query(QueryPlan::Project(ProjectPlan {
            input: ProjectInputPlan::Source(SourcePlan::Derived(DerivedSourcePlan {
                query: Box::new(QueryPlan::Project(ProjectPlan {
                    input: ProjectInputPlan::LeftOuterJoin(Box::new(join)),
                    projection: wildcard.clone(),
                })),
    ...
    fn ast_errors() {
        ...
        let actual = QueryNode::LeftOuterHashJoinNode(
            table("A")
                .select()
                .left_join("B")
                .hash_executor("B.id", "A.id"),
