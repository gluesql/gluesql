...
#[derive(Clone, Debug)]
pub(in crate::query_builder::select) enum PrevNode<'a> {
    Select(SelectNode<'a>),
    Join(Box<JoinNode<'a>>),
...
impl<'a> JoinNode<'a> {
    pub(in crate::query_builder::select) fn new<N: Into<PrevNode<'a>>>(
        prev_node: N,
        name: String,
        alias: Option<String>,
        join_operator_type: JoinOperatorType,
...
mod tests {
    use {
        crate::query_builder::{Build, table, test_query_builder},
