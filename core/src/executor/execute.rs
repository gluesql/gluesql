use {
    super::{
        ...
        update::Update,
        validate::{validate_check_constraint, validate_unique, ColumnValidation},
    },
    crate::{
        ast::{
...
async fn execute_inner<T: GStore + GStoreMut>(
    ...
    match statement {
        ...
        Statement::Update {
            ...
            let foreign_keys = Rc::new(schema.foreign_keys.as_slice());
            ...
            if let (Some(column_defs), Some(all_columns)) =
                (schema.column_defs.as_ref(), all_columns.as_ref())
            {
                ...
                validate_unique(storage, table_name, column_validation, rows.clone()).await?;
                validate_check_constraint(storage, &schema, all_columns, rows).await?;
            }
