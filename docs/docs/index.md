...
1. [Breaking the Boundary between SQL and NoSQL Database](articles/breaking-the-boundary-between-sql-and-nosql.md)
2. [Revolutionizing Databases by Unifying Query Interfaces](articles/revolutionizing-databases-by-unifying-query-interfaces.md)
3. [Test-Driven Documentation - Automating User Manual Creation](articles/test-driven-documentation.md)
...
### Sled Storage
Sled Storage is a persistent data storage option for GlueSQL that is built on the Sled key-value embedded database in Rust. It supports non-clustered indexes and transactions. Sled Storage is an excellent choice for handling and storing data in a Rust environment. To use Sled Storage, you can create a SledStorage instance using a path.

### Redb Storage
Redb Storage leverages the [redb](https://docs.rs/redb) embedded database for persistent storage with non-clustered index and transaction support. It keeps all tables in a single database file, offering another simple way to keep your data on disk while enjoying GlueSQL's query capabilities.
