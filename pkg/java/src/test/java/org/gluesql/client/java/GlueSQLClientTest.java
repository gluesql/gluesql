...
public class GlueSQLClientTest {
  ...
  void testAsynchronousQuery() throws Exception {
    ...
    CompletableFuture<List<QueryResult>> future =
        client.queryAsync("CREATE TABLE products (id INTEGER, name TEXT)");

    List<QueryResult> result = future.get(5, TimeUnit.SECONDS);
    assertNotNull(result);

    // Insert data asynchronously
    CompletableFuture<List<QueryResult>> insertFuture =
        client.queryAsync("INSERT INTO products VALUES (1, 'Laptop')");
  ...
  void testStorageFactory() {
    ...
    SledConfig sledConfig =
        SledConfigBuilder.create("/tmp/test-advanced.sled")
            .cacheCapacity(2048L)
