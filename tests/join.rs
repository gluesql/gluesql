...
fn join() {
    ...
    let select_sqls = [
        ...
        /* TODO:
        (2, "SELECT * FROM Item i LEFT JOIN Player p ON p.id = i.player_id AND i.quantity = 1;"),
        (2, "SELECT * FROM Item LEFT JOIN Player ON Player.id = Item.player_id AND Item.quantity = 1;"),
        */
    ];
